<?PHP
	...
	class segment {
		...
		public function getView($scope) {
			...
			if(isset($this->options['templateUrl'])) { $view->setTemplateUrl($this->options['templateUrl']); }
			if(isset($this->options['controllerUrl'])) { $ctrl->setFile( $this->options['controllerUrl'] ); }
			if(isset($this->options['controller'])) { $ctrl->setClosure( $this->options['controller'] ); }
